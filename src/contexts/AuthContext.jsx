...
const AuthProvider = (props) => {
  ...
  useEffect(() => {
    function recuperarDadosArmazenados() {
      ...
      if (alunoArmazenado && tokenArmazenado) {
        ...
        if (location.pathname === "/cadastro" || location.pathname === "/nova-senha" || location.pathname === "/nova-solicitacao") return
        else navigate('/')
      }
    }
  ...
  const handleLogin = ({email, senha}, setLoading, setMostrandoAlert, setDados) => {
    setLoading(true)
  ...
  return (
      <AuthContext.Provider value={{autenticado, aluno, handleLogin, handleLogout}}>
          {props.children}
